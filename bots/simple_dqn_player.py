...
class SimpleDQNPlayer(EnvPlayer):
    ...
    def __init__(self, num_battles=10000, **kwargs):
        ...
        for key, klass, supported in sets:
            ...
            else: self._knowledge[key] = list(map(lambda x: x.name.lower().replace("_", ""),
    ...
    def _create_model(self):
        ...
        self._dqn = DQNAgent(
            ...
            policy=self._policy,
            memory=self._memory,
            nb_steps_warmup=max(1000, int(self.num_battles / 10)), # The number of battles we go through before we start training: https://hub.packtpub.com/build-reinforcement-learning-agent-in-keras-tutorial/
